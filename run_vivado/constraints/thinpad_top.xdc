...
set_property -quiet CLOCK_DEDICATED_ROUTE FALSE [get_nets -quiet clk_11M0592_IBUF]
...
set_property -quiet CLOCK_DEDICATED_ROUTE FALSE [get_nets -quiet clock_btn_IBUF]
set_property -quiet CLOCK_DEDICATED_ROUTE FALSE [get_nets -quiet reset_btn_IBUF]
...
# CPU and SRAM clocks are generated by the same PLL and remain related in the
# timing model.  Do not hide all paths between them with a clock-group exception.
# Any protocol-proven CDC exception is added narrowly at the synchronizer or
# dual-clock FIFO boundary in cdc_protocol_exceptions.xdc.  That file is read
# late during implementation, after the synthesized CDC objects exist.
